...
{
  if (LoadConfigData(GetDir(DIR_ELFS_CONFIG|MEM_EXTERNAL),L"StatusClock.bcfg")<0)
  {
    if (LoadConfigData(GetDir(DIR_ELFS_CONFIG|MEM_INTERNAL),L"StatusClock.bcfg")<0)
    {
      if (SaveConfigData(GetDir(DIR_ELFS_CONFIG|MEM_EXTERNAL),L"StatusClock.bcfg")<0)
      {
        SaveConfigData(GetDir(DIR_ELFS_CONFIG|MEM_INTERNAL),L"StatusClock.bcfg");
      }
    }
  }
};
