...
void ICBTestConnManager::OnConnected(int ConnectionID,int AccountID,void * ClientData)
{
	...
	memset(&local_socket_address,0,sizeof(sockaddr_in));
	...
	if (pISocketManager) pISocketManager->Release();
	if (pISocketUtility) pISocketUtility->Release();
	if (pIHostNameResolver) pIHostNameResolver->Release();
	if (pISocketUserAgent) pISocketUserAgent->Release();
